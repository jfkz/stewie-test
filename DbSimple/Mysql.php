...
class DbSimple_Mysql extends DbSimple_Generic_Database
{
	...
	function DbSimple_Mysql($dsn)
	{
		...
		if (!$ok) {
			return $this->_setDbError('mysqli_connect("' . $str . '", "' . $p['user'] . '")');
		}
		if (isset($p["charset"])) {
			mysqli_query($this->link, 'SET NAMES ?', $p["charset"]);
		}
	}
	...
	function _performGetBlobFieldNames($result)
	{
		...
		for ($i = mysqli_num_fields($result) - 1; $i >= 0; $i--) {
			...
			if (strpos($type, "BLOB") !== false) {
				$blobFields[] = mysqli_field_name($result, $i);
			}
		}
	...
	function _performQuery($queryMain)
	{
		...
		} elseif (preg_match('/^\s* SELECT  \s+/six', $queryMain[0])) {
